...
//include_once($_SERVER['DOCUMENT_ROOT']."/includes-es/function_check_mail.php");	
	...
	mysql_select_db('borse_contactos', $link);
		...
		$query="SELECT  count(*) as total  FROM registro_cotizaciones WHERE correo = '$email'";
		...
		$query="SELECT MAX(numero) as maximo FROM registro_cotizaciones WHERE correo = '$email'";	
		$result =  mysql_query($query);
		...
		$max=$row["maximo"];
			...
			$queryas="SELECT  id,asesor FROM registro_cotizaciones WHERE numero = '$max'";
...
$Fecha=date("d/m/Y H:i:s");	
			...
			$fecha2=explode(" ",$fechareg);
			
			
			if ($fecha1[0]==$fecha2[0]){
				
				echo "<script>					
		$(window.location).attr('href', '/contacto-es/cotizacion-gracias.php'); 
		</script>";
			}else{
